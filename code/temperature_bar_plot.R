...
annotation <- t_data %>% 
    ...
    slice(1, n()) %>% 
    mutate(t_diff = 0,
           x = year + c(-5, 5)) # Makes the year at the zero level

t_data %>% 
    ggplot(aes(x = year, y = t_diff, fill = t_diff)) +
    geom_col(show.legend = FALSE) +
    geom_text(data = annotation, aes(x = x,
                                     label = year),
              color = 'white') +
    geom_text(x = 1880, y = 1, hjust = 0,
              label = glue("Global temperature have increased by over {
                           max_t_diff}\u00B0C since {min(annotation$year)}"),
              color = 'white') +
    ...
    #                      values = rescale(c(min(t_data$t_diff), 
    #                                         0,
    #                                         max(t_data$t_diff)
    #                                         )
    #                                       ),
    #                      limits = c(min(t_data$t_diff), 
    #                                 max(t_data$t_diff))) +
...
ggsave('figures/temperature_bar_plot.png',
       width = 7,
       height = 4)
